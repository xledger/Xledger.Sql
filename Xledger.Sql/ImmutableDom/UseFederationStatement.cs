...
namespace Xledger.Sql.ImmutableDom {
    public class UseFederationStatement : TSqlStatement, IEquatable<UseFederationStatement> {
        ...
        public override int CompareTo(TSqlFragment that) {
            ...
            compare = Comparer.DefaultInvariant.Compare(this.federationName, othr.federationName);
            if (compare != 0) { return compare; }
            compare = Comparer.DefaultInvariant.Compare(this.distributionName, othr.distributionName);
            if (compare != 0) { return compare; }
            compare = Comparer.DefaultInvariant.Compare(this.@value, othr.@value);
            if (compare != 0) { return compare; }
            compare = Comparer.DefaultInvariant.Compare(this.filtering, othr.filtering);
            if (compare != 0) { return compare; }
            return compare;
        } 
        
        public static bool operator < (UseFederationStatement left, UseFederationStatement right) => Comparer.DefaultInvariant.Compare(left, right) <  0;
        public static bool operator <=(UseFederationStatement left, UseFederationStatement right) => Comparer.DefaultInvariant.Compare(left, right) <= 0;
        public static bool operator > (UseFederationStatement left, UseFederationStatement right) => Comparer.DefaultInvariant.Compare(left, right) >  0;
        public static bool operator >=(UseFederationStatement left, UseFederationStatement right) => Comparer.DefaultInvariant.Compare(left, right) >= 0;
        ...
        public static UseFederationStatement FromMutable(ScriptDom.UseFederationStatement fragment) {
            ...
            if (fragment.GetType() != typeof(ScriptDom.UseFederationStatement)) { throw new NotImplementedException("Unexpected subtype of UseFederationStatement not implemented: " + fragment.GetType().Name + ". Regenerate immutable type library."); }
            return new UseFederationStatement(
                federationName: ImmutableDom.Identifier.FromMutable(fragment.FederationName),
