...
namespace Xledger.Sql.ImmutableDom {
    public class GraphMatchNodeExpression : BooleanExpression, IEquatable<GraphMatchNodeExpression> {
        ...
        public override int CompareTo(TSqlFragment that) {
            ...
            compare = StructuralComparisons.StructuralComparer.Compare(this.node, othr.node);
            if (compare != 0) { return compare; }
            compare = StructuralComparisons.StructuralComparer.Compare(this.usesLastNode, othr.usesLastNode);
            if (compare != 0) { return compare; }
            return compare;
        } 
