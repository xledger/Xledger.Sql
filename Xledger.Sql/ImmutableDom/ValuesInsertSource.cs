...
namespace Xledger.Sql.ImmutableDom {
    public class ValuesInsertSource : InsertSource, IEquatable<ValuesInsertSource> {
        ...
        public override int CompareTo(TSqlFragment that) {
            ...
            compare = Comparer.DefaultInvariant.Compare(this.isDefaultValues, othr.isDefaultValues);
            if (compare != 0) { return compare; }
            compare = Comparer.DefaultInvariant.Compare(this.rowValues, othr.rowValues);
            if (compare != 0) { return compare; }
            return compare;
        } 
        public static bool operator < (ValuesInsertSource left, ValuesInsertSource right) => Comparer.DefaultInvariant.Compare(left, right) <  0;
        public static bool operator <=(ValuesInsertSource left, ValuesInsertSource right) => Comparer.DefaultInvariant.Compare(left, right) <= 0;
        public static bool operator > (ValuesInsertSource left, ValuesInsertSource right) => Comparer.DefaultInvariant.Compare(left, right) >  0;
        public static bool operator >=(ValuesInsertSource left, ValuesInsertSource right) => Comparer.DefaultInvariant.Compare(left, right) >= 0;
