...
namespace Xledger.Sql.ImmutableDom {
    public class AtTimeZoneCall : PrimaryExpression, IEquatable<AtTimeZoneCall> {
        ...
        public override int CompareTo(TSqlFragment that) {
            ...
            compare = StructuralComparisons.StructuralComparer.Compare(this.dateValue, othr.dateValue);
            if (compare != 0) { return compare; }
            compare = StructuralComparisons.StructuralComparer.Compare(this.timeZone, othr.timeZone);
            if (compare != 0) { return compare; }
